...
| `WGEN-PIPELINE-EQUIVALENCE-001` | `EXP-WGEN-001`, `EXP-WGEN-005`, `EXP-WGEN-006` | `Reopened` | `RequiredDifferential` | Same-input normalized semantic identity is required |
...
| Phase 2 — Region-native local and distributed runtime | `Complete` | [Region-owned state](../development/region-state.md), [tick pipeline](../development/region-tick-pipeline.md), [local runtime](../development/local-region-runtime.md), [recovery](../development/persistence-recovery.md), [Lattice adapter](../development/lattice-adapter.md), [multi-node deployment](../development/multi-node-deployment.md), and [topology conformance](../development/topology-conformance.md) | Twelve Regions converge for 10,000 ticks across local, in-process, and three-process topologies; recovery-point portability, fencing, faults, and overload outcomes pass. Position-independent production storage remains Goal 07 scope. |
...
| 2026-08-02 | `G01-D021` | `Accepted` | Replace the former world-generation equivalence boundary with same-input normalized semantic identity against the locked official 26.2 server. Keep the old machine manifests frozen as historical evidence until `G01-P8-B4` versions their schema and renderer. | [Vanilla exactness contract](../development/worldgen-equivalence-boundary.md) and `G01-P8-B4` |
| 2026-08-02 | `G01-D022` | `Accepted` | Goal 01's topology harness proves portable recovery bytes and fencing, not a location-independent production storage backend. Per-node files cannot be the durable authority when Region placement changes; Goal 07 owns shared storage, storage-side fencing, and recovery after permanent source-disk loss. | [ADR-0026](../adr/0026-location-independent-region-storage.md) and Goal 07 |
...
- [x] The three non-worldgen unresolved observations remain explicit `DeferredExperiment` records in the historical [completion boundary](../reports/goal-01/g01-p10-b6-completion-record.md#completion-boundary).
- [ ] Same-input world generation matches the locked official 26.2 server under the [vanilla exactness contract](../development/worldgen-equivalence-boundary.md).
