...
| Level and chunk lifecycle | `net.minecraft.server.level.ServerLevel#save`, `net.minecraft.server.level.ServerLevel#unload`, `net.minecraft.server.level.ServerChunkCache#save`, `net.minecraft.server.level.ServerChunkCache#close`, `net.minecraft.server.level.ChunkMap#saveAllChunks`, `net.minecraft.server.level.ChunkMap#processUnloads`, `net.minecraft.server.level.ChunkMap#scheduleUnload`, `net.minecraft.server.level.ChunkMap#scheduleChunkLoad` | The mapped WorldLifecycle inventory fixes dirty/admission caps, current save-dependency identity, unload cancellation, mark-unloaded/synchronous-save-snapshot/block-entity-teardown order, POI/light closure and first ready/send publication. Ordinary unload waits for a stable holder save-sync future, marks a LevelChunk unloaded, flushes POI, clears dirty through `tryMarkSaved`, copies `SerializableChunkData` on the server thread and admits asynchronous encode/write; it then clears block entities/ticks/debug/light without waiting for durable write completion. A synchronous save exception or later asynchronous write failure is reported but does not restore dirty state, retain the live chunk or schedule an unload-path retry. Missing or missing-level chunk data creates an empty ProtoChunk; reported non-Error read/parse failure logs and also creates empty, while an `Error` becomes a crash report. `WGEN-PIPELINE-001` owns persisted status, section/palette, heightmap, structure, light and postprocessing reconstruction. |
...
| Reconstructed and transient state | `net.minecraft.server.level.ServerChunkCache#onChunkReadyToSend`, `net.minecraft.server.network.PlayerChunkSender#dropChunk`, `net.minecraft.server.network.PlayerChunkSender#sendNextChunks`, `net.minecraft.server.players.PlayerList#placeNewPlayer`, `net.minecraft.server.players.PlayerList#sendLevelInfo` | Every completion leaf classifies its behavior state as serialized, code/data-derived, session-local or transient. Serialization includes committed world/entity/player/block-entity/item/menu/progression state and queued work explicitly named by those owners. Code/data-derived registries, tags, recipes, loot, models and behavior tables bind from their owning active data views. Reset scope is boundary-specific: leaving a tracking view removes a pending unsent chunk without a packet or forgets an already-sent live chunk, but it does not recreate the connection or reset connection-wide batch acknowledgements, prediction state or menus. Disconnect/rejoin and play-to-configuration removal/re-entry create new player/play-listener/chunk-sender state and therefore reset their runtime IDs, acknowledgement windows, interpolation, open menus and sent mirrors. A later full-chunk packet is constructed from the ready chunk at send time and is not a replay of the earlier client mirror. No interrupted callback resumes or catches up. |
...
- Runtime-only transport IDs, acknowledgement windows, interpolation caches and client mirrors are
  not persistent identities, but their lifetime follows their owner. A single chunk forget/reset
  does not reset connection-wide state; player/connection/reconfiguration reconstruction does.
  Every reset must still lead to a fresh authoritative reprojection at the owner's next boundary.
...
1. Replay each persisted, reconstructed and transient field through clean chunk unload/reload,
   player disconnect/rejoin, play-to-configuration removal/re-entry and full restart. At each
   boundary assert only the owner-scoped mirrors reset, then compare the first authoritative
   publication plus next admitted callback with an uninterrupted control.
...
7. Inject saved-data encoding and IO failures across one and multiple files; verify dirty-clear
   ...
   joining.
8. Pause ordinary unload after the synchronous `SerializableChunkData` copy, then fail encoding and
   asynchronous storage separately. Assert live teardown does not wait for durability, dirty state
   is not restored and no unload-path retry occurs. Crash between admitted writes and compare
   restart with a clean save/close: only completed writes may survive, and no interrupted RNG draw,
   iterator, callback, transport state or client mirror may resume unless its owner persists it.
