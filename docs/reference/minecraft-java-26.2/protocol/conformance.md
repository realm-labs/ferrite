...
| `C4-CREATIVE-SLOT` | Exercise every signed-short slot, empty/max/over-count/feature-disabled stacks, survival/creative, valid inventory slots and negative drops through throttle saturation/leak. | Mutate/broadcast only slots 1..45 for infinite-material players with valid data, no-op slot zero/other nonnegative values, drop valid negative requests below threshold and warn/no-op excess without resetting idle time. |
