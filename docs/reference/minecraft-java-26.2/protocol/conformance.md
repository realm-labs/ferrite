...
| `C4-GAMETEST-PRESENTATION` | Run relative-position command for multiple players; query/init test blocks with known/unknown tests while target screens are open/closed/replaced. | Send highlight/status only to the requester, preserve absolute/relative coordinates and optional size rules, mutate only matching active presentation and create no test-world mutation or action acknowledgement. |
| `C4-LOW-DISK-WARNING` | Report usable space one byte below/equal/above 67,108,864 after full save and chunk failures on integrated/base/dedicated servers with mixed permissions; repeat notifications. | Trigger below only, log on base, globally send only from dedicated server to administrators, add/update client toast for every packet and send no ACK. |
| `C4-LIVE-TAGS-CODECS` | Reuse configuration tag fixtures with empty/multiple registries, tags, duplicates, signed/out-of-range raw members, malformed identifiers/counts/truncation/trailing data. | Decode the identical grammar, resolve every member through current configured registries and reject missing/out-of-range/malformed values without opaque retention. |
| `C4-LIVE-TAGS-RESOLUTION` | Mix valid and invalid registries/members across map entries on remote and in-memory connections, then repeat valid replacements. | Prepare all registries before applying so lookup failure preserves every old binding; apply valid remote sets, skip in-memory binding and recompute fuel/creative search state after successful handling. |
