...
| `C4-OPERATOR-BLOCK-PERMISSIONS` | Invoke each tool across instabuild/operator/command-block-enabled combinations and missing/wrong targets. | Require both instabuild and command-game-master, send only command-tool denial messaging, reproduce silent/wrong-target no-ops and gate command update execution/messages on server enablement. |
| `C4-COMMAND-BLOCK-TOOLS` | Change command block modes/conditional/automatic/tracking/commands and command minecart command/tracking, including empty command and disabled command blocks. | Preserve facing/conditional mode conversion, clear last output when tracking turns off, update correct entities and emit only documented nonempty-command messages/updates. |
