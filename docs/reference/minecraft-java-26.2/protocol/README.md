...
interaction/session feedback and motion projection are source-specified; entity spawn/state,
effects, inventory, command and other families plus C4 remain incomplete. Missing later-level
details remain implementation blockers and must not be inferred from another Minecraft version or
from memory.
