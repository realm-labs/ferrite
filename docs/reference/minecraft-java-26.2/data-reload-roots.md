...
| Reload family | Locked source roots | Exact ordering, ownership and failure conclusion |
|---|---|---|
| Pack discovery, selection and feature flags | `net.minecraft.server.packs.repository.PackRepository#reload`, `net.minecraft.server.packs.repository.PackRepository#setSelected`, `net.minecraft.server.MinecraftServer#reloadResources`, `net.minecraft.server.MinecraftServer#configurePackRepository`, `net.minecraft.world.level.WorldDataConfiguration#enabledFeatures` | Discovery is source-order overwrite into an ID-sorted map; repository `setSelected` preserves requested order for present IDs, silently drops missing IDs, then inserts every required pack at its configured position. Bootstrap logs missing enabled packs, honors explicit disables, auto-adds eligible source packs, removes selected packs whose requested flags exceed the allowed set, forces vanilla only when empty, then adds feature-source packs needed by forced flags. Compatibility metadata is not a selection gate here. Live candidate construction instead opens only the supplied present IDs in supplied order; after swap, repository selection is rebuilt from the original request and can insert a required pack that an adversarial direct request omitted from the candidate. The world feature set remains unchanged. Publication replaces the in-memory `WorldDataConfiguration`; only a later world save makes that pack selection durable. |
| Bootstrap and worldgen registries | `net.minecraft.resources.RegistryDataLoader#load` | Resource and network overloads share one dependency-aware loader over the exact worldgen, dimension or synchronized registry list. Registry tasks run concurrently against a combined context that exposes all loading registries for holder resolution; only after every task completes are all registries frozen, validated in list order and wrapped as one immutable access. Decode, duplicate/reference, freeze or validator errors are collected by exact key, logged in sorted registry/key order and raised as one `ReportedException`; no partial access is returned or published. `WGEN-002`, `WGEN-003` and subtype leaves own values and first consumers. |
| Reloadable registries and loot | `net.minecraft.server.ReloadableServerRegistries#reload`, `net.minecraft.server.ReloadableServerRegistries$LoadResult#lookupWithUpdatedTags`, `net.minecraft.server.ReloadableServerRegistries$Holder#getLootTable`, `net.minecraft.world.level.storage.loot.LootDataType#runValidation` | Predicate, item-modifier and loot-table registries load concurrently into fresh registries using the candidate pending-tag lookup, then freeze and replace only the reloadable registry layer. Malformed JSON/codec entries log and are omitted; validation of context, references and recursion logs warnings but does not reject the candidate. An unexpected task/freeze failure fails candidate construction. Missing table lookup returns `LootTable.EMPTY`. All callers use the one full candidate lookup, while loot/content leaves own evaluation and committed outcomes. |
| Listener construction and dependency barrier | `net.minecraft.server.ReloadableServerResources#loadResources`, `net.minecraft.server.ReloadableServerResources#listeners`, `net.minecraft.server.packs.resources.SimpleReloadInstance#create`, `net.minecraft.server.packs.resources.ReloadInstance#done` | Reloadable registries finish first, then component initializers build on the background executor. The exact listener list is recipe manager, function library, advancement manager. Their preparation work may overlap on the background executor; each barrier waits for every listener to finish preparation and for the preceding listener's complete apply future, so apply runs on the server executor strictly recipes, functions, advancements. `Util.sequenceFailFast` exposes the first exceptional listener completion; unfinished work is not published, and profiling changes measurement only. |
| Tags and component rebinding | `net.minecraft.tags.TagLoader#load`, `net.minecraft.tags.TagLoader#build`, `net.minecraft.tags.TagLoader#loadTagsForExistingRegistries`, `net.minecraft.server.ReloadableServerResources#updateComponentsAndStaticRegistryTags` | Tag resource stacks apply low-to-high pack order; `replace` clears earlier entries, final values preserve first insertion while deduplicating. Bad files log and contribute nothing. Dependency sorting resolves required/optional element and tag references; missing required references or cycles log and omit the affected tag, while optional misses do not fail it. A nonempty pending set replaces that registry's complete live tag map and refreshes each existing holder; an entirely empty loaded map creates no pending object and therefore leaves that registry's prior live tags unchanged. Candidate lookups expose pending bindings during decode. After server publication, pending static tags apply first and newly built data components second. Tag-backed and component-bearing leaves own subsequent reads. |
| Recipes, functions and advancements | `net.minecraft.world.item.crafting.RecipeManager#prepare`, `net.minecraft.world.item.crafting.RecipeManager#apply`, `net.minecraft.world.item.crafting.RecipeManager#finalizeRecipeLoading`, `net.minecraft.server.ServerFunctionLibrary#reload`, `net.minecraft.server.ServerAdvancementManager#apply` | Each JSON ID uses the highest-priority matching resource; malformed recipe or advancement entries log and are omitted. Recipes publish one `RecipeMap`, then post-publication finalization rebuilds feature-filtered ingredient properties, stonecutter entries, displays and identity-keyed reverse indices, skipping impossible non-special recipes with a warning. Functions and their tag files prepare concurrently; each function compiles independently against the candidate dispatcher, failures log and are omitted, then tags bind against the successfully compiled map. Advancements validate with warnings, replace the map, rebuild parent/tree/display layout, and later cause every player's old progress to be saved, cleared and reloaded/revalidated against the new manager. Semantic leaves own matching, execution, rewards and retained progress. |
| Server publication and live refresh | `net.minecraft.server.MinecraftServer#reloadResources`, `net.minecraft.server.players.PlayerList#reloadResources`, `net.minecraft.commands.Commands#sendCommands` | Candidate construction never mutates `resources`; any open/load/listener failure completes exceptionally, closes the constructed candidate manager when it exists and retains the old live snapshot. Publication is one server-executor task with this observable prefix: close old resources; install candidate; rebuild repository selection; replace in-memory world data configuration with unchanged features; apply static tags/components; finalize recipes; save all players; reload every retained advancement tracker; broadcast live tags; for each current player send synchronized recipes then its initial recipe book; replace the function library; retarget and clear the structure-template cache; rebuild fuel values. No other server task interleaves, but these distinct owner publications are not one atomic snapshot. There is no rollback after the pointer swap: an injected exception leaves the completed prefix exposed, and later server tasks can observe a hybrid of new and not-yet-replaced managers. `/reload` builds a new dispatcher but does not call `sendCommands`, so already-playing clients retain their previous command tree until another owner sends one; server execution and later joins use the installed dispatcher. |
| Active-session reconfiguration and convergence | `net.minecraft.server.network.ServerGamePacketListenerImpl#switchToConfig`, `net.minecraft.server.players.PlayerList#reloadResources` | Candidate preparation leaves active play on the old resources. The publication task serializes against server-thread join, dimension and reconfiguration tasks: a task before publication uses the old state, while a task after successful completion uses the completed prefix. After a post-swap failure, later tasks observe that retained partial prefix rather than an inferred complete snapshot. Ordinary live reload targets only players still in `PlayerList`, reloads their server advancement trackers before queuing live tags/recipes/books, does not immediately flush advancement packets, and does not force configuration. A player already removed for reconfiguration is skipped by that live refresh and instead converges through the configuration registry/tag pipeline and later play entry against the then-current managers. The locked reconfiguration families own acknowledgements, disconnects and re-entry. |
...
- Reloadable registries are prepared before the resource listener aggregate is constructed. The
  candidate remains isolated through listener completion; after the pointer swap, tags/components,
  recipes, player refresh, functions, structures and fuel publish in distinct ordered steps with
  no cross-step rollback or atomic-snapshot guarantee.
...
- Bootstrap worldgen/dimension registry values and generator instances are not reconstructed by
  live reload. Static registry tags can rebind existing holders, while loot registries, recipes,
  functions, advancements, components, structures and fuel follow their separate live owners.
  Existing worlds, objects and sessions can therefore retain or live-read different views; data
  decoding alone is insufficient without publication-prefix and convergence checks.
...
- `ITM-SMITHING-TEMPLATE-001` owns nineteen duplication recipes, eighteen trim recipes/pattern
  records, twelve netherite transforms, base and trade-rebalance acquisition tables, forty-nine
...
1. Bootstrap with required, missing, incompatible, disabled, auto-enabled and feature-source packs
   in multiple orders; compare selected order, warnings, enabled/disabled persistence and feature
   closure. Repeat live reload with missing and directly omitted required IDs; distinguish the
   candidate's opened pack list from post-swap repository selection and later-save durability, and
   prove that the world feature set remains unchanged.
2. Load every bootstrap/network registry list with cross-registry references and inject decode,
   duplicate, freeze and validator failures; verify one immutable result or one sorted aggregate
   failure with no partial publication.
3. Reload valid, malformed, missing and recursively invalid predicates, modifiers and loot tables;
   verify omitted parses, warning-only validation, `LootTable.EMPTY` fallback and one coherent
   candidate lookup using pending tags.
4. Gate each of recipe, function and advancement preparation and apply independently; verify
   overlapping preparation, the recipes-functions-advancements apply chain, fail-fast completion
   and candidate cleanup before publication.
5. Stack tag files with merge/replace, duplicates, optional/required misses, cycles, bad JSON,
   an empty registry tag directory and a nonempty replacement; verify exact binding, holder refresh
   and the empty-map retention branch.
6. Inject failure at pack open, candidate load, each listener phase and every publication step.
   Pre-swap failures must retain the old snapshot; post-swap failures must expose the documented
   hybrid prefix without rollback, including pack configuration before and after a later save.
7. During successful and injected post-swap-failed reloads, queue old-world callbacks, player join,
   dimension change and play-to-configuration acknowledgement before and after the publication
   task. Verify the old state or exact completed prefix selected by task order, then advancement-
   tracker reload, tags, per-player recipe/book pairs and later authoritative play projection.
8. Compare an already-playing client, a new join and a player removed for reconfiguration. Only
   retained live players receive reload-time tags/recipes/books; advancement packets wait for their
   ordinary flush, removed players converge through configuration, and new/re-entering clients use
   the then-current dispatcher. `/reload` itself sends no command-tree update to live clients.
