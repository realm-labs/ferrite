...
|---|---|---|
| Pack discovery, selection and feature flags | `net.minecraft.server.packs.repository.PackRepository#reload`, `net.minecraft.server.packs.repository.PackRepository#setSelected`, `net.minecraft.server.MinecraftServer#configurePackRepository`, `net.minecraft.world.level.WorldDataConfiguration#enabledFeatures` | Discovery is source-order overwrite into an ID-sorted map; reload rebuilds the prior selection and `setSelected` preserves requested order for present IDs, silently drops missing IDs, then inserts every required pack at its configured position. Bootstrap logs missing enabled packs, honors explicit disables, auto-adds eligible source packs, removes selected packs whose requested flags exceed the allowed set, forces vanilla only when empty, then adds feature-source packs needed by forced flags. Compatibility metadata is not a selection gate here. Live reload opens the supplied present IDs in supplied order and retains the world's existing feature set; it does not enable newly requested features. |
...
| Server publication and live refresh | `net.minecraft.server.MinecraftServer#reloadResources`, `net.minecraft.server.players.PlayerList#reloadResources`, `net.minecraft.commands.Commands#sendCommands` | Candidate construction never mutates `resources`; any open/load/listener failure completes exceptionally, closes the constructed candidate manager when it exists and retains the old live snapshot. Successful publication is one server-executor task: close old resources; install candidate; persist selected packs with unchanged features; apply static tags/components; finalize recipes; save all players; reload all advancement trackers; broadcast live tags; send each player synchronized recipes then its initial recipe book; replace the function library; reload structure templates; rebuild fuel values. No other server task interleaves. There is no rollback after the pointer swap if an injected publication/refresh exception occurs. `/reload` builds a new dispatcher but does not call `sendCommands`, so already-playing clients retain their previous command tree until another owner sends one; server execution and later joins use the new dispatcher. |
| Active-session reconfiguration and convergence | `net.minecraft.server.network.ServerGamePacketListenerImpl#switchToConfig`, `net.minecraft.server.players.PlayerList#reloadResources` | Candidate preparation leaves active play on the old snapshot. The publication task serializes against join, dimension and reconfiguration tasks, so each such task observes either the complete pre-publication or post-publication server snapshot. Ordinary live reload uses the tag/recipe packets above and does not force play clients into configuration. A separately admitted switch removes the player from live world state, sends start-configuration, changes outbound protocol, rejects ordinary play messages while waiting and accepts only the configuration acknowledgement that installs the configuration listener. The locked reconfiguration families own registry/tag selection, acknowledgements, disconnects and re-entry; later play entry sends the active command tree and authoritative world state. |
...
- Reloadable registries are prepared before the resource listener aggregate is constructed. The
  candidate remains isolated through listener completion; the exact server-thread publication
  order and its post-swap no-rollback boundary are fixed by the matrix above.
...
1. Bootstrap with required, missing, incompatible, disabled, auto-enabled and feature-source packs
   ...
   closure, then repeat live reload to prove that its feature set remains unchanged.
...
7. During a successful reload, interleave old-world callbacks, player join, dimension change and
   play-to-configuration acknowledgement. Verify task-order selection of one complete snapshot,
   then tags, recipes, recipe book, advancement refresh and later authoritative play projection.
8. Compare an already-playing client, a new join and a reconfigured client. The live client gets no
   command-tree resend from `/reload`; new/re-entering clients and server execution use the new
   dispatcher, while the protocol vectors own registry/tag acknowledgements and failure branches.
