...
The slice ledger currently has no `Todo` or `InProgress` entries, but the catalog has 221 explicitly
`Unreviewed` IDs across two recoverable fallback families. The surface ledger additionally has seven
