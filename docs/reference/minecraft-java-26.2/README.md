...
The surface ledger has five `InProgress` roots and no `Todo` roots, so gameplay readiness is
intentionally blocked only by surface ownership work. Five roots are structurally `Mapped`; this
