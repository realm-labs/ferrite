...
The surface ledger has two `InProgress` roots and no `Todo` roots, so gameplay readiness is
intentionally blocked only by surface ownership work. Eight roots are structurally `Mapped`; this
