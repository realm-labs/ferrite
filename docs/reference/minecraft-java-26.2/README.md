...
The slice ledger currently has no `Todo` or `InProgress` entries, but the catalog has 244 explicitly
`Unreviewed` IDs across three recoverable fallback families. The surface ledger additionally has seven
