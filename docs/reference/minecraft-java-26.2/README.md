...
`Unreviewed` IDs across six recoverable fallback families. The surface ledger additionally has two
`Todo` and six `InProgress` roots, so gameplay readiness is intentionally blocked for both reasons.
