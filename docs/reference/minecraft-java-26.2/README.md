...
The surface ledger has six `InProgress` roots and no `Todo` roots, so gameplay readiness is
intentionally blocked only by surface ownership work. Four roots are structurally `Mapped`; this
