...
`Unreviewed` IDs across six recoverable fallback families. The surface ledger additionally has one
`Todo` and seven `InProgress` roots, so gameplay readiness is intentionally blocked for both reasons.
