...
- Implementation-level leaf rules: 143, spread across all ten subsystems.
- Directed experiment definitions: 96; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,013 across 719 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 781 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 315 blocks, 245 items, 37 entity types and 184
