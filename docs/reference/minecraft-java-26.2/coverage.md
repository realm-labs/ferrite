...
- Implementation-level leaf rules: 134, spread across all ten subsystems.
- Directed experiment definitions: 87; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,838 across 659 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 809 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 336 blocks, 7 block-entity types, 245 items,
