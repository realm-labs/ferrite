...
- Implementation-level leaf rules: 180, spread across all ten subsystems.
- Directed experiment definitions: 133; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,299 across 811 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 650 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 184 blocks, 245 items, 37 entity types and 184
