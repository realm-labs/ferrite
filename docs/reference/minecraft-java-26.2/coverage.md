...
- Implementation-level leaf rules: 190, spread across all ten subsystems.
- Directed experiment definitions: 143; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,432 across 845 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 545 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 87 blocks, 237 items, 37 entity types and 184
