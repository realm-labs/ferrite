...
- Implementation-level leaf rules: 157, spread across all ten subsystems.
- Directed experiment definitions: 110; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,179 across 774 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 697 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 231 blocks, 245 items, 37 entity types and 184
