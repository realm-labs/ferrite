...
- Implementation-level leaf rules: 188, spread across all ten subsystems.
- Directed experiment definitions: 141; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,413 across 836 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 561 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 95 blocks, 245 items, 37 entity types and 184
