...
- Implementation-level leaf rules: 177, spread across all ten subsystems.
- Directed experiment definitions: 130; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,256 across 796 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 656 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 190 blocks, 245 items, 37 entity types and 184
