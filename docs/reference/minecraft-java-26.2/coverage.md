...
- Implementation-level leaf rules: 165, spread across all ten subsystems.
- Directed experiment definitions: 118; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,233 across 791 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 675 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 209 blocks, 245 items, 37 entity types and 184
