...
- Implementation-level leaf rules: 152, spread across all ten subsystems.
- Directed experiment definitions: 105; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,142 across 760 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 759 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 293 blocks, 245 items, 37 entity types and 184
