...
- Implementation-level leaf rules: 181, spread across all ten subsystems.
- Directed experiment definitions: 134; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,311 across 814 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 642 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 176 blocks, 245 items, 37 entity types and 184
