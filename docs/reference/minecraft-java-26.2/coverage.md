...
- Implementation-level leaf rules: 154, spread across all ten subsystems.
- Directed experiment definitions: 107; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,165 across 769 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 726 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 260 blocks, 245 items, 37 entity types and 184
