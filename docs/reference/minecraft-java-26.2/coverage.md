...
- Implementation-level leaf rules: 172, spread across all ten subsystems.
- Directed experiment definitions: 125; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
...
current controlling rules. The catalog currently exposes 661 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 195 blocks, 245 items, 37 entity types and 184
