...
- Implementation-level leaf rules: 192, spread across all ten subsystems.
- Directed experiment definitions: 145; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,446 across 849 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 542 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 86 blocks, 235 items, 37 entity types and 184
