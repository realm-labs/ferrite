...
- Implementation-level leaf rules: 156, spread across all ten subsystems.
- Directed experiment definitions: 109; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,178 across 774 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 705 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 239 blocks, 245 items, 37 entity types and 184
