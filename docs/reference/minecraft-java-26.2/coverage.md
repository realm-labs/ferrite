...
- Implementation-level leaf rules: 131, spread across all ten subsystems.
- Directed experiment definitions: 84; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,800 across 641 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 815 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 336 blocks, 10 block-entity types, 248 items,
