...
- Implementation-level leaf rules: 195, spread across all ten subsystems.
- Directed experiment definitions: 148; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,475 across 859 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 536 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 84 blocks, 231 items, 37 entity types and 184
