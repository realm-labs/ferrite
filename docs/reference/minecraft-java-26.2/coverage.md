...
- Implementation-level leaf rules: 142, spread across all ten subsystems.
- Directed experiment definitions: 95; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,995 across 711 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 782 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 316 blocks, 245 items, 37 entity types and 184
