...
- Implementation-level leaf rules: 175, spread across all ten subsystems.
- Directed experiment definitions: 128; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,245 across 793 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 658 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 192 blocks, 245 items, 37 entity types and 184
