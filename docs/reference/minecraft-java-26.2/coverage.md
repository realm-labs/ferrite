...
- Implementation-level leaf rules: 155, spread across all ten subsystems.
- Directed experiment definitions: 108; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,173 across 773 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 710 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 244 blocks, 245 items, 37 entity types and 184
