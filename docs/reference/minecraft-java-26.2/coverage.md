...
- Implementation-level leaf rules: 92, spread across all ten subsystems.
- Directed experiment definitions: 48; all currently `planned`, so none is incorrectly counted as confirming evidence.
- Source locators: 1,179 across 467 classes, verified by `javap -p -s` against locked jars and the locked fastutil dependency used by prediction reconciliation.
