...
- Implementation-level leaf rules: 139, spread across all ten subsystems.
- Directed experiment definitions: 92; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,946 across 695 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 788 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 321 blocks, 1 block-entity type, 245 items,
