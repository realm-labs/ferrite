...
- Implementation-level leaf rules: 173, spread across all ten subsystems.
- Directed experiment definitions: 126; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
...
current controlling rules. The catalog currently exposes 660 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 194 blocks, 245 items, 37 entity types and 184
