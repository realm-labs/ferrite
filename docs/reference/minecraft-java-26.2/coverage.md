...
- Implementation-level leaf rules: 178, spread across all ten subsystems.
- Directed experiment definitions: 131; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,272 across 800 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 655 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 189 blocks, 245 items, 37 entity types and 184
