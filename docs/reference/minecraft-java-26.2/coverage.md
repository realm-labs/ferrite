...
- Implementation-level leaf rules: 161, spread across all ten subsystems.
- Directed experiment definitions: 114; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,210 across 784 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 679 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 213 blocks, 245 items, 37 entity types and 184
