...
- Implementation-level leaf rules: 163, spread across all ten subsystems.
- Directed experiment definitions: 116; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,218 across 788 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 677 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 211 blocks, 245 items, 37 entity types and 184
