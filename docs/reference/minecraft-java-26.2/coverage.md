...
- Implementation-level leaf rules: 159, spread across all ten subsystems.
- Directed experiment definitions: 112; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,185 across 778 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 687 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 221 blocks, 245 items, 37 entity types and 184
