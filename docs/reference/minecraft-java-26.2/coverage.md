...
- Implementation-level leaf rules: 184, spread across all ten subsystems.
- Directed experiment definitions: 137; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,360 across 829 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 595 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 129 blocks, 245 items, 37 entity types and 184
