...
- Implementation-level leaf rules: 158, spread across all ten subsystems.
- Directed experiment definitions: 111; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,180 across 775 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 691 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 225 blocks, 245 items, 37 entity types and 184
