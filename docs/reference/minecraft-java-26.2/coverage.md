...
- Implementation-level leaf rules: 140, spread across all ten subsystems.
- Directed experiment definitions: 93; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,970 across 701 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 786 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 320 blocks, 245 items, 37 entity types and 184
