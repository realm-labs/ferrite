...
- Implementation-level leaf rules: 189, spread across all ten subsystems.
- Directed experiment definitions: 142; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,422 across 842 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 555 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 91 blocks, 243 items, 37 entity types and 184
