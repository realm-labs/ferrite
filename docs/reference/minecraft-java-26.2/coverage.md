...
- Implementation-level leaf rules: 115, spread across all ten subsystems.
- Directed experiment definitions: 68; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,697 across 598 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 840 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 341 blocks, 16 block-entity types, 14 game
