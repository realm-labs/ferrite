...
- Implementation-level leaf rules: 118, spread across all ten subsystems.
- Directed experiment definitions: 71; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,716 across 610 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 825 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 338 blocks, 15 block-entity types, three game
