...
- Implementation-level leaf rules: 179, spread across all ten subsystems.
- Directed experiment definitions: 132; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,291 across 807 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 651 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 185 blocks, 245 items, 37 entity types and 184
