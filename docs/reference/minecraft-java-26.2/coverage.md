...
- Implementation-level leaf rules: 151, spread across all ten subsystems.
- Directed experiment definitions: 104; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,137 across 756 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 775 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 309 blocks, 245 items, 37 entity types and 184
