...
- Implementation-level leaf rules: 186, spread across all ten subsystems.
- Directed experiment definitions: 139; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,393 across 834 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 563 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 97 blocks, 245 items, 37 entity types and 184
