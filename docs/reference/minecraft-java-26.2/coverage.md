...
- Implementation-level leaf rules: 148, spread across all ten subsystems.
- Directed experiment definitions: 101; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,060 across 736 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 776 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 310 blocks, 245 items, 37 entity types and 184
