...
- Implementation-level leaf rules: 193, spread across all ten subsystems.
- Directed experiment definitions: 146; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,457 across 853 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 541 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 86 blocks, 234 items, 37 entity types and 184
