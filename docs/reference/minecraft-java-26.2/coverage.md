...
- Implementation-level leaf rules: 153, spread across all ten subsystems.
- Directed experiment definitions: 106; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,150 across 761 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 743 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 277 blocks, 245 items, 37 entity types and 184
