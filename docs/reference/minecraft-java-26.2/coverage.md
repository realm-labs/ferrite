...
- Implementation-level leaf rules: 135, spread across all ten subsystems.
- Directed experiment definitions: 88; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,856 across 664 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 807 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 336 blocks, 5 block-entity types, 245 items,
