...
- Implementation-level leaf rules: 194, spread across all ten subsystems.
- Directed experiment definitions: 147; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,467 across 858 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 540 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 86 blocks, 233 items, 37 entity types and 184
