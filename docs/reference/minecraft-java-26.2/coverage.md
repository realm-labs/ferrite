...
- Implementation-level leaf rules: 147, spread across all ten subsystems.
- Directed experiment definitions: 100; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,041 across 731 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 777 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 311 blocks, 245 items, 37 entity types and 184
