...
- Implementation-level leaf rules: 141, spread across all ten subsystems.
- Directed experiment definitions: 94; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,978 across 704 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 785 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 319 blocks, 245 items, 37 entity types and 184
