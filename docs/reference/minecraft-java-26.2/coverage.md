...
- Implementation-level leaf rules: 191, spread across all ten subsystems.
- Directed experiment definitions: 144; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,438 across 848 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 543 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 86 blocks, 236 items, 37 entity types and 184
