...
- Implementation-level leaf rules: 138, spread across all ten subsystems.
- Directed experiment definitions: 91; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,925 across 690 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 790 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 322 blocks, 2 block-entity types, 245 items,
