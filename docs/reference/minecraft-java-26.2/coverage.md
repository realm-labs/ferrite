...
- Implementation-level leaf rules: 130, spread across all ten subsystems.
- Directed experiment definitions: 83; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,781 across 636 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 816 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 336 blocks, 11 block-entity types, 248 items,
