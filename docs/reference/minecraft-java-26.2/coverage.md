...
- Implementation-level leaf rules: 176, spread across all ten subsystems.
- Directed experiment definitions: 129; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,255 across 796 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 657 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 191 blocks, 245 items, 37 entity types and 184
