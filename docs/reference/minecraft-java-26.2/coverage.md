...
- Implementation-level leaf rules: 185, spread across all ten subsystems.
- Directed experiment definitions: 138; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,375 across 832 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 571 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 105 blocks, 245 items, 37 entity types and 184
