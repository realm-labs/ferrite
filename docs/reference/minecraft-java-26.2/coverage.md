...
- Implementation-level leaf rules: 146, spread across all ten subsystems.
- Directed experiment definitions: 99; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,029 across 727 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 778 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 312 blocks, 245 items, 37 entity types and 184
