...
- Implementation-level leaf rules: 144, spread across all ten subsystems.
- Directed experiment definitions: 97; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,019 across 722 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 780 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 314 blocks, 245 items, 37 entity types and 184
