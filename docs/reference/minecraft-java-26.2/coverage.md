...
- Implementation-level leaf rules: 132, spread across all ten subsystems.
- Directed experiment definitions: 85; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 1,816 across 644 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 814 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 336 blocks, 9 block-entity types, 248 items,
