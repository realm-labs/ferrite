...
- Implementation-level leaf rules: 164, spread across all ten subsystems.
- Directed experiment definitions: 117; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,223 across 790 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 676 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 210 blocks, 245 items, 37 entity types and 184
