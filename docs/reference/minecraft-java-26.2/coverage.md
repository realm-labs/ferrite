...
- Implementation-level leaf rules: 145, spread across all ten subsystems.
- Directed experiment definitions: 98; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,026 across 726 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 779 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 313 blocks, 245 items, 37 entity types and 184
