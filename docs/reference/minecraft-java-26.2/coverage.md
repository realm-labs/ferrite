...
- Implementation-level leaf rules: 160, spread across all ten subsystems.
- Directed experiment definitions: 113; all currently `planned`, so none is incorrectly counted as
  confirming evidence.
- Source locators: 2,209 across 783 classes, verified by `javap -p -s` against locked jars and the
...
current controlling rules. The catalog currently exposes 682 `Unreviewed` IDs instead of hiding
them behind broad `Special` or `DataOnly` fallbacks: 216 blocks, 245 items, 37 entity types and 184
