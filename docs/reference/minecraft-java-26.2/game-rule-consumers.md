...
| `entity_drops` | `VehicleEntity#destroy`, `Painting#dropItem`, `ContainerEntity#chestVehicleDestroyed`, `ItemFrame#dropItem`, `Leashable#tickLeash`, `FallingBlockEntity#tick`, `CopperGolem#turnToStatue` | `ENT-ENTITY-DROPS-001`: all seven differently placed gates, pre-read mutations, guarded non-item effects, RNG and projection are explicit. |
...
| `max_block_modifications` | `CloneCommands#clone`, `FillCommand#fillBlocks`, `FillBiomeCommand#fill` | `BLK-COMMAND-AREA-001`: all three inclusive long-volume gates, validation order, modes/filters/strict, load boundaries, ordered commits, partial failure and feedback counts are explicit. |
| `max_command_forks` | `Commands#executeCommandInContext` | `SIM-COMMAND-LIMIT-001`: outer-context snapshot and strict ordinary-redirect cardinality rejection, error audiences, custom bypass and no-truncation/result behavior are explicit. |
| `max_command_sequence_length` | `Commands#executeCommandInContext`, `CommandBlock#executeChain` | `SIM-COMMAND-LIMIT-001`: clamped action quota and independent raw chain-step snapshot, all cost sites, exhaustion/logging and mid-run rule changes are explicit. |
...
| `raids` | `Raids#tick`, `Raids#createOrExtendRaid` | `MOB-RAID-001`: both live reads, consumed omen admission, managed retirement, waves, persistence reconstruction and client projection are explicit. |
