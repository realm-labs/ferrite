...
0 blocks, 0 items, 0 entity types and 0 worldgen records. All 49 block-entity types now have
...
`BLK-POLISHED-BASALT-001` owns axis states 7003..7005, its 1.25/4.2 correct-pickaxe
black/base-drum/Basalt-sound profile, two Basalt conversions, slow-bouncy selection, 905 raw
Ancient-City/Bastion cells plus three Bastion connector finals and oriented cube-column projection.
`BLK-MUDDY-MANGROVE-ROOTS-001` owns axis states 165..167, its 0.7 Podzol/dedicated-sound
full-cube profile, universal self harvest, Mud/Roots conversion, six block and two item tags,
33 transitive mud memberships, frog/dripleaf/Sniffer/Enderman selectors, regular equipment, both
mangrove generators, one reachable Trial cell and oriented cube-column projection.
`BLK-SMOOTH-STONE-001` owns state 13480, its 2/6 any-Pickaxe Stone profile, Furnace production,
slab and Blast-Furnace sinks, Mason chest acquisition, slow-bouncy equipment, 59 raw cells across
five ordinary/zombie Village payloads and its opaque cube projection.
`BLK-END-STONE-001` owns End Stone and End Stone Bricks states 9477/14796, their shared 3/9
any-Pickaxe Stone profile, twelve masonry/Eye-template recipes, End-Stone-only Dragon/Sculk/chorus
and End terrain/portal/feature joins, both slow-bouncy items, zero End-Stone template cells, 576
Bricks cells across eight reachable End-city templates and both opaque cube projections.
`BLK-BLACKSTONE-001` owns six full-block Blackstone states 21831/22242..22245/22656, their
COLOR_BLACK any-Pickaxe profiles, Gilded loot/Piglin/sound branches, 39 exact and eight tag-keyed
recipes, Nether selectors and deterministic ruined-portal mapping, 146,042 raw Bastion cells,
twelve ordered degradation lists and exact block/item projections.
`BLK-TUFF-001` owns five full-block Tuff states 23452/23863/24274/24275/24686, their 1.5/6
any-Pickaxe profiles, 38 recipes, two Trial chest rows and the Trial location icon, base/deepslate
ore and composed terrain joins, iron-vein and sulfur-spring generation, 129,849 raw spring/Trial
cells, Trial connector/protection paths and exact block/item projections.
`BLK-PRISMARINE-001` owns three full-block Prismarine states 12631..12633, their 1.5/6
any-Pickaxe/Stone profiles, 18 recipes including Tide-template duplication, hard-coded Conduit
frame admission, procedural Monument roles, 31 raw cells across six cold large-ocean-ruin
templates, slow-bouncy equipment and exact static/animated block-item projections.
`BLK-SPONGE-001` owns dry/wet states 560/561, their 0.6 full-cube/Hoe/sound profiles, exact
...
`BLK-SMALL-MUSHROOM-001` replaces Brown Mushroom's overbroad light-emitter ownership and closes
both property-free states 2336/2337: exact light/support survival, color-local cap-five random
spread, bonemeal huge growth, loot/Composter/pot/recipe/entity/horse/trade/Enderman joins, ten
placed profiles, five Fallen Tree providers, 16 raw structure cells, procedural hut placement and
cross/sprite projection.
`BLK-NETHER-FUNGUS-001` closes Crimson/Warped Fungus states 20975/20958: their shared 14-block
reloadable support expansion, color-matched-nylium bonemeal and planted huge-feature dispatch,
self/pot/Composter/chest/recipe/Hoglin/Strider/Enderman joins, four weighted vegetation inputs,
two count-eight huge-fungus profiles, zero raw structure cells and cross/sprite projection.
`BLK-NYLIUM-001` closes Crimson/Warped Nylium states 20974/20957: exact upward-dampening decay,
always-successful asymmetric bonemeal feature dispatch, Netherrack conversion, Silk/Netherrack
loot, Crimson chest/Hoglin joins, six direct block tags, slow-bouncy equipment, Nether surface
selection, huge/vegetation/vine inputs, zero raw structure cells and cube projection.
`BLK-NETHER-VINES-001` closes Weeping/Twisting head/body states 20977..21030: directional
support and conversion, age/probability random growth, decreasing-threshold Bone Meal lengthening,
head-only item/loot/Composter acquisition, climb/glide/fall-message joins, regular/bonemeal/huge
generation, four zero-cell template censuses and untinted cross/body-texture item projection.
`BLK-NETHERRACK-001` closes state 6997: ordered skylight/tag/exact-color Bone Meal conversion,
correct-tool self loot and Furnace/template recipes, infiniburn/Wither-Rose/composed-tag/Sulfur-Cube
joins, Nether terrain/replacement/spring/fire/support paths, 1,876 raw template cells plus four
post-processor Jigsaw finals, and 16-rotation cube projection.
`BLK-NETHER-PLANKS-001` closes Crimson/Warped Planks states 21032/21033: ordinary full-cube
mining/self loot, explicit Fire and final fuel exclusion, 60 unique recipes with exact mixed-color
semantics and unlocks, six Wood-tool plus Shield repair, bouncy Sulfur-Cube/dispenser joins, zero
cells across all 1,212 templates and exact cube/tab projection.
`BLK-MOSSY-COBBLESTONE-001` closes state 3368: correct-Pickaxe self loot, two producers and seven
exact sinks, slow-bouncy equipment/dispenser joins, forest-rock/dungeon/Jungle-Temple production,
125 exact Village processor entries, 369 raw cells across 20 Outpost/Trial/Ocean-Ruin/Village
templates, dormant Nether-portal replacement and exact cube/tab projection.
`BLK-BUBBLE-COLUMN-001` closes states 15294/15295: exact full-source-water propagation, scheduled
base/update semantics, bucket pickup, generic/projectile/boat motion, breathing/path/speed/enchanting
joins, client fluid/particle/sound projection and zero item/block-entity/template/worldgen payloads.
`BLK-CACTUS-001` closes ages/states 0..15/6929..6944: exact support, delayed removal,
...
`BLK-CARVED-PUMPKIN-001` closes facing states 7019..7022: placement and shared Snow/Iron/Copper
Golem patterns with non-atomic removal/insertion/chest ordering; Pumpkin carving, dispenser,
helmet/map/gaze/waypoint/HUD, Sulfur Cube, Halloween and Enderman paths; recipe/loot/advancement/
Composter joins, three raw cells across Outpost/Mansion templates and orientable projection.
`BLK-SULFUR-CINNABAR-001` closes the eight property-free raw, Polished, Brick and Chiseled
Sulfur/Cinnabar blocks: copied physical profiles, correct-Pickaxe self loot, a 70-recipe/
...
`BLK-DIRT-SUBSTRATE-001` closes Grass Block, Dirt, Coarse Dirt, Podzol, Mycelium and Rooted Dirt:
their exact state/physical/snow profiles, Grass/Mycelium decay and spread, both bonemeal paths,
Shovel/Hoe/Water transformations, loot/recipe/trade/mob/tag joins, worldgen ownership, 8,461 raw
template cells plus 526 executable Jigsaw final states, and tint/model projection.
`BLK-AGRICULTURAL-GROUND-001` closes Dirt Path and Farmland: their exact reduced-height support,
placement and delayed-decay asymmetries, 162-cell irrigation, drying and trampling, Hoe/loot/crop/
Farmer joins, three street processors, 8,311 raw template cells plus 53 Dirt-Path Jigsaw final
states, legacy migration and dry/moist/rotated projection.
`BLK-SNOW-FAMILY-001` closes Snow, Snow Block and Powder Snow: eight-layer support, stacking and
light melt; dynamic collision/contact/fall, freezing and Skeleton conversion; bucket/cauldron,
Snow-Golem, weather, loot/recipe/progression/tag/worldgen joins; 2,525 raw template cells and eight
Snow-Block Jigsaw final states; legacy migration and layer/shell/fog/overlay projection.
`BLK-MOSS-001` closes Moss Block and Moss Carpet: full/thin geometry, nonair Carpet support and
Bone-Meal patch dispatch; loot/recipe/compost/chest/trade/Sniffer/Enderman/Sulfur-Cube joins; a
38-tag Moss-Block closure, vegetation/Mangrove/fungus selection, five raw Trial-Chambers cells,
identity persistence and shared-texture projection.
`BLK-MUD-001` closes state 30415: full selection/support versus 14/16 collision, always-valid
spawn and nonconductor/view/suffocation predicates; Water-Potion production and dripstone-to-Clay
conversion; loot/recipe/Snow/plant/mob/regular-equipment joins; complete 40/3 block/item-tag
closures, surface/disk/Mangrove generation, 13 raw cells and exact full-cube client projection.
`BLK-PACKED-ICE-001` closes state 12914: ordinary full-cube mechanics with friction 0.98,
Silk-only self loot, Ice/Packed-Ice/Blue-Ice compression, chest/trader acquisition, complete
7/2 block/item-tag closures, fast-sliding equipment, Frozen-Peaks/Frozen-Ocean and nine feature/
structure joins, 268 raw cells plus two Jigsaw final states, legacy migration and cube-all projection.
...
`ITM-STICK-001` replaces the loose Stick fallback. It owns eleven foliage tables, Dead Bush,
Witch, fishing, Desert-Well archaeology and five chest rows, two producing and 109 consuming
...
`ITM-TURTLE-SCUTE-001` replaces the final loose-item fallback. It owns the one-shot
mob-drops-gated Turtle adulthood gift, Turtle-Helmet crafting and material repair, exact
Helmet/Water-Breathing/brewing joins, Cleric and Leatherworker purchases, zero-template census
and flat/equipment projection while generic aging, loot, crafting, anvil, equipment, effects,
brewing, merchant, structure and client algorithms retain owners.
`BLK-COBWEB-001` replaces the Cobweb block fallback. It owns deferred contact drag and fall reset,
Spider/Cave-Spider exemption, correct Shears/Sword harvest and Cobweb/String loot, hardcoded
...
`ENT-COD-001` replaces the Cod entity fallback. It owns transient exact-class schooling and its
...
`ENT-TROPICAL-FISH-001` replaces the Tropical Fish entity fallback. It owns the packed
pattern/base-color/pattern-color state, 22 common and arbitrary rare variants, same-variant
eight-member natural schools versus solitary rare finalization, five spawn rows with Lush-Cave
height bypass, three-component bucket transfer and tooltip, Tropical-Fish loot/tags/sounds, seven
migration contexts and two-mesh/two-tint rendering while shared fish, spawn, bucket, damage, loot,
tooltip and render algorithms retain owners.
`ENT-PUFFERFISH-001` replaces the Pufferfish entity fallback. It owns raw three-state inflation,
one-tick-delayed threat response, two-stage inflate/deflate timing, through-wall and
invisibility-insensitive scary-entity selection, distinct Mob and Player sting paths, three
warm-ocean spawn rows, intentionally state-losing bucket transfer, Pufferfish loot/tags/sounds,
seven migration contexts and malformed-state-sensitive three-model rendering while shared fish,
spawn, bucket, damage, effect, loot and render algorithms retain owners.
`ENT-TADPOLE-001` replaces the Tadpole entity fallback. It owns raw server-only age and synchronized
locking, Slime-Ball acceleration, Golden-Dandelion resets, Frog conversion and variant finalization,
simultaneous fish goals and Brain activities, Frogspawn-only production, age-preserving buckets,
empty loot/XP, five tags, four migration contexts and one water-sensitive tail model while shared
fish, Brain, conversion, Frogspawn, bucket, damage and render algorithms retain owners.
`ENT-SQUID-001` replaces the Squid entity fallback. It owns the two concurrent control-free goals,
phase-driven propulsion and rotation, retained-attacker damage result and thirty-packet ink response,
...
`ENT-ELDER-GUARDIAN-001` replaces the Elder Guardian entity fallback. It owns persistent home,
Guardian movement and defensive thorns, a 60-tick synced beam, phased Mining-Fatigue pulses,
code-built three-Elder Monument production versus zero biome rows, five-pool loot, five direct
tags, hostile-mob criteria, Egg, eight migration contexts and exact entity/beam/apparition
projection while generic AI, damage, effects, spawning, structure insertion, loot, advancement,
particle and render algorithms retain owners.
`ENT-EVOKER-001` replaces the Evoker entity fallback. It owns the exact Raider/Illager goal graph,
three-spell arbitration, Vex summoning, close/far fang construction, Wololo recoloring, raid and
four-marker Mansion production versus zero biome rows, Totem/Emerald loot, two direct tags,
hostile-mob criteria, Egg, seven migration contexts and exact entity/fang/item projection while
generic AI, damage, spawning, raid/structure insertion, projectile damage, loot, advancement,
particle and render algorithms retain owners.
`ENT-GHAST-001` replaces the Ghast entity fallback. It owns three-goal float/look/fireball
arbitration, collision-swept acceleration, charging metadata, signed explosion power, the
Player-reflected Large-Fireball damage exception, three Nether-biome rows, cluster-one spawning,
...
`ENT-ILLUSIONER-001` replaces the Illusioner entity fallback. It owns the inherited-then-local
nine-goal graph, the shared spellcaster machine with its mirror and blindness admission, the strict
effective-difficulty gate and non-repeating target latch, spawn-issued Bow ranged fire, the entirely
client-side four-image illusion matrix, zero biome rows with no raid, patrol, structure or Spawn-Egg
...
owners.
`ENT-PARCHED-001` replaces the Parched entity fallback. It owns the halved bow cadence, the Weakness
instance attached to plain arrows only and carried through pickup and re-fire, the reciprocal
Weakness immunity, the single Desert biome row, the latched Husk-driven Camel-Husk jockey, three
...
`WGEN-PIPELINE-001`. The 133 placed vegetation records are also exact data-only inputs after the
eleven-modifier, 23-chain, 624-instance and 90-configured-holder audit in that owner. The nine flat
generator presets are exact data-only inputs after the settings/source/UI codec audit, 33-layer and
18-structure-reference census, visible-tag check and payload digest. No locked catalog record remains
`Unreviewed`. Registry entries outside these gameplay categories remain discoverable in
