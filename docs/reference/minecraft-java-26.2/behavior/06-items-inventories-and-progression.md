...
- **Verification owner (`ITM-CONTAINER-001`; `EXP-ITM-002`):** Generate a transition table for every `ContainerInput` variant, invalid index, remote close, and delayed reordering.
...
- **Verification owner (`ITM-CRAFT-001`; `EXP-ITM-003`):** Generate golden cases for every recipe serializer, emphasizing component-sensitive ingredients, remainder overflow, and shift-click loop termination.
