...
- **Verification owners:** `BLK-UPDATE-001` and `EXP-BLK-002` lock retention, pre-removal suppression, listener/ticker rebinding, ordered first/last-tick semantics and activity gates. `BLK-TRIAL-SPAWNER-001` and `EXP-BLK-006` own the trial-spawner subtype's complete runtime state machine; other subtype callbacks remain content-owned.
