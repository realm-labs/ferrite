...
`BLK-SAPLING-001`/`EXP-BLK-074` fixes states 29..44 and a centered 12-by-12-by-12 selection with
empty collision/occlusion, instant break and `DESTROY` piston reaction. All states survive only
over the 11-member `supports_vegetation` closure; any directional shape update returns air after
support loss. Stage zero advances with an ignored flags-260 write, while stage one owns the exact
small/flower/mega clear-place-restore transaction.
`BLK-BAMBOO-001`/`EXP-BLK-075` fixes sapling state 15278 and stalk states 15279..15290, their
offset selection/collision columns and one-item dual-form placement. Both require the 17-member
`supports_bamboo` closure; sapling loss is immediate, stalk loss schedules delay one, and growth
owns the exact age/leaves/stage and ignored flags-3 segment writes through height 16.
`BLK-ANCIENT-DEBRIS-001`/`EXP-BLK-076` fixes property-free state 21819 as an ordinary full cube
with map color `COLOR_BLACK`, default harp, Ancient Debris sounds, strength 30.0/1200.0 and diamond-tier
pickaxe drops. It adds no block entity, state transition, random tick or specialized placement,
update, collision or piston callback.
`BLK-STEM-CROP-001`/`EXP-BLK-077` fixes states 8334..8357: four attached facings and age 0..7
for each fruit. Both stems require exact farmland, while fruit attachment supports close through
the eleven-member vegetation substrate; support loss is immediate. Losing the faced fruit first
...
potatoes 10667..10674 and beetroots 14811..14814. Every state requires raw brightness at least 8
plus exact farmland below; loss returns air immediately. Growth requires brightness 9, uses the
shared 3x3 farmland/crowding speed and flags-2 next-age write, with beetroot's separate
...
`BLK-TORCHFLOWER-CROP-001`/`EXP-BLK-079` fixes crop states 14797..14798 and mature flower 2323.
