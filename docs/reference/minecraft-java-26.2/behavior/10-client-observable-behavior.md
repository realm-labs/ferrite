...
- **Verification owner (`CLI-EFFECT-001`, `ITM-ENDER-CHEST-001`; `EXP-CLI-003`, `EXP-ITM-008`):** The ender-chest leaf fixes its block-event lid interpolation, sounds and ambient portal-particle draw sites. Classify every remaining gameplay event as required, settings-droppable, or prediction-deduplicated, then record a vanilla client to verify relative tick and duplicate suppression.
