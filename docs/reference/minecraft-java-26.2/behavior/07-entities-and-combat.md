...
- **Open verification:** Lock first/last tick for spawn/remove during ticking, UUID collision, and re-add ordering after load.
...
- **Open verification:** Lock nested-passenger order, switching mounts during tick, chunk unload, and dismount collision candidates.
...
- **Open verification:** Concrete boat/minecart constants, entity traversal order, and simultaneous multi-entity pushing need trajectory GameTests, so this aggregate rule remains `Cross-checked`.
...
- **Open verification:** Lock block/entity ties, multiple targets in one tick, remaining displacement after deflection, and unloaded-chunk edges.
...
- **Open verification:** A data-driven matrix must lock every damage tag's bypass layers and floating-point rounding. Full numeric order still needs black-box cross-checking, hence `Cross-checked`.
...
- **Open verification:** Build state-machine fixtures for strong-short over weak-long, hidden chains, infinite effects, and multiple additions in one tick.
...
- **Open verification:** Lock relative ticks of loot, equipment, XP, advancement, and removal, plus idempotency of repeated damage after death starts.
...
- **Open verification:** Build a black-box matrix for every relative flag, cross-dimension identity, passenger policy, and target-load failure. This remains `Cross-checked`.
