...
`EXP-BLK-054`, `EXP-BLK-055`, `EXP-BLK-056`, `EXP-BLK-057`, `EXP-BLK-058`, `EXP-BLK-059`, `EXP-BLK-060`, `EXP-BLK-061`, `EXP-BLK-062`, `EXP-BLK-063`, `EXP-BLK-064`, `EXP-BLK-065`, `EXP-BLK-066`, `EXP-BLK-067`, `EXP-BLK-068`, `EXP-BLK-069`, `EXP-BLK-070`, `EXP-BLK-071`, `EXP-BLK-072`, `EXP-BLK-073`, `EXP-BLK-074`, `EXP-BLK-075`, `EXP-BLK-076`, `EXP-BLK-077`, `EXP-BLK-078`, `EXP-BLK-079`
...
`COLOR_ORANGE`.
`BLK-STONE-VARIANT-001` fixes the same full-solid boundary for states 2..7: no skylight
propagation, dampening 15, shade brightness 0.2, emission 0 and no identity-dependent fluid/light
callback. Granite, diorite and andesite families use `DIRT`, `QUARTZ` and `STONE` respectively.
`BLK-STONE-BRICK-001` fixes the same full-solid boundary for states 7754..7757: no skylight
propagation, dampening 15, shade brightness 0.2, emission 0, map color `STONE` and no
identity-dependent fluid/light callback.
`BLK-BEACON-STORAGE-001` fixes the same full-solid boundary for states 2338, 2339, 5309, 9727 and
21818: no skylight propagation, dampening 15, shade brightness 0.2 and emission 0. Their map colors
are `GOLD`, `METAL`, `DIAMOND`, `EMERALD` and `COLOR_BLACK`; none adds an identity-dependent
fluid/light callback.
`BLK-RAW-STORAGE-001` fixes the same full-solid boundary for states 32070..32072: no skylight
propagation, dampening 15, shade brightness 0.2 and emission 0. Map colors are `RAW_IRON`,
`COLOR_ORANGE` and `GOLD`; none adds an identity-dependent fluid/light callback.
...
`BLK-AMETHYST-BLOCK-001` fixes the same full-solid boundary for state 23402: no skylight
propagation, dampening 15, shade brightness 0.2, emission 0, map color `COLOR_PURPLE` and no
identity-dependent fluid/light callback. Its chime and resonance sounds do not emit block light.
`BLK-BUDDING-AMETHYST-001` fixes non-occluding small/medium/large/cluster emission `1/2/4/5`,
directional shapes and exact source-water fluid state. A waterlogged neighbor update schedules the
ordinary water delay before support-loss removal; generic light rebuild and fluid execution remain
with the environment owners.
`BLK-CALCITE-SMOOTH-BASALT-001` fixes full-solid emission-zero states 27160 and 32069 with false
skylight propagation, dampening 15 and shade 0.2. Calcite and smooth basalt use
`TERRACOTTA_WHITE` and `COLOR_BLACK`; neither adds an identity-dependent fluid/light callback.
...
`BLK-BRICKS-001` fixes the same ordinary opaque boundary for state 2340 with `COLOR_RED` map
color; its structure-selected instances do not add a fluid or lighting callback.
`BLK-PACKED-MUD-001` fixes that boundary for state 7758 with `DIRT` map color; raw and
processor-produced trail instances add no fluid or lighting callback.
`BLK-MUD-BRICKS-001` fixes that boundary for state 7759 with `TERRACOTTA_LIGHT_GRAY`; raw,
connector-produced and aging-surviving trail instances add no fluid or lighting callback.
`BLK-PURPUR-BLOCK-001` fixes the same opaque boundary for state 14712 with `COLOR_MAGENTA`;
End-city instances add no fluid or lighting callback.
`BLK-RED-NETHER-BRICKS-001` fixes that opaque boundary for state 14847 with `NETHER`; its audited
zero-worldgen identity adds no fluid or lighting callback.
`BLK-NETHER-WART-BLOCK-001` fixes the same opaque boundary for state 14846 with `COLOR_RED`;
surface, fungus, vines and carver-selected instances add no fluid or lighting callback.
`BLK-WARPED-WART-BLOCK-001` fixes that boundary for state 20959 with `WARPED_WART_BLOCK`;
warped surface/fungus, twisting-support and carver contexts add no fluid or lighting callback.
`BLK-NETHER-SPROUTS-001` fixes the transparent vegetation boundary: state 20961 has emission and
dampening zero, empty collision/occlusion and an empty fluid state, so skylight propagates. Fluid
replacement removes rather than waterlogs it; the offset cross does not become an occluding shape.
`BLK-NETHER-ROOTS-001` fixes the same transparent vegetation boundary for states 20960/21031:
emission/dampening zero, empty collision/occlusion, skylight propagation and replacement rather
than waterlogging. Potted states 21829/21828 instead retain their small collision shape while
disabling occlusion; neither form adds a fluid state or environmental callback.
`BLK-NETHER-WART-001` fixes emission/dampening zero, empty collision/occlusion and skylight
propagation for all four crop ages. The block retains no fluid state and its growth test has no
light, biome, humidity or dimension predicate; support and generic fluid replacement remain
separate boundaries.
`BLK-NETHER-STEM-001` fixes emission zero, dampening 15, full collision/occlusion and false
skylight propagation for all 24 axis states. Axis changes texture orientation but not light,
support, path or environmental predicates.
`BLK-CORAL-BLOCK-001` fixes emission zero, dampening 15, full collision/occlusion, false skylight
propagation and shade brightness 0.2 for all ten states. They have no waterlogged property or
center fluid; only a neighboring fluid state's `water` membership keeps a live block from
scheduling or completing dry conversion. Flowing water and a waterlogged neighbor qualify.
`BLK-CORAL-PLANT-001` fixes empty collision, emission zero and dry skylight propagation for all
120 states. Waterlogged true exposes source water and wins the live scan before neighbors; a
supported live update requests the generic water tick twice through override plus base, while a
dead update requests it once. Scheduler deduplication owns the equivalent requests.
`BLK-FLOWER-POT-001` fixes no-occlusion light behavior for all 37 states: dampening zero, shade
brightness 1.0, skylight propagation and server emission zero. Potted open eyeblossom's model
contains full-bright faces, but that client material does not create world block light.
`BLK-COPPER-FULL-001` fixes ordinary full-solid light behavior for all 24 states: dampening 15,
shade brightness 0.2, no skylight propagation and server emission zero. Weather age selects map
colors orange, light gray terracotta, warped stem and warped nylium; wax and collection do not
change those four age projections.
`BLK-SAPLING-001` fixes the same no-occlusion light boundary for states 29..44: emission and
dampening zero, shade brightness 1 and skylight propagation. Stage does not change light; map
color is Plant except cherry Pink and pale oak Metal.
`BLK-BAMBOO-001` fixes emission and dampening zero with skylight propagation for all 13 states.
Sapling map color is Wood, stalk map color Plant; age/leaves/stage affect model or shape but never
world-light emission.
`BLK-ANCIENT-DEBRIS-001` fixes the ordinary full-solid boundary for state 21819: no skylight
propagation, dampening 15, shade brightness 0.2, emission 0, map color `COLOR_BLACK` and no
identity-dependent fluid or light callback.
...
`BLK-TORCHFLOWER-CROP-001`/`EXP-BLK-079` fixes emission and dampening zero, skylight propagation,
empty collision and map color Plant for crop states 14797..14798 and flower state 2323. The crop
has fixed centered six-pixel-wide shapes of height 6/10; the flower has height 10 and a deterministic
XZ offset. Neither state adds a fluid or light callback.
`BLK-PITCHER-CROP-001`/`EXP-BLK-080` fixes emission/dampening zero, skylight propagation, Plant map
...
**Owners:** `ENV-FIRE-001`, `BLK-SHELF-001`, `BLK-BEDROCK-001`, `BLK-NETHER-STEM-001`,
`BLK-CORAL-BLOCK-001`, `BLK-CORAL-PLANT-001`, `BLK-FLOWER-POT-001`, `BLK-COPPER-FULL-001`,
`BLK-SAPLING-001`, `BLK-BAMBOO-001`, `BLK-ANCIENT-DEBRIS-001`, `BLK-STEM-CROP-001`,
`BLK-OVERWORLD-CROP-001`, `BLK-TORCHFLOWER-CROP-001`,
`BLK-SOUL-SAND-001`, `BLK-MAGMA-001`;
`EXP-ENV-003`, `EXP-BLK-013`, `EXP-BLK-031`, `EXP-BLK-037`, `EXP-BLK-038`, `EXP-BLK-069`,
`EXP-BLK-070`, `EXP-BLK-071`, `EXP-BLK-072`, `EXP-BLK-073`, `EXP-BLK-074`, `EXP-BLK-075`,
...
`BLK-NETHER-STEM-001` fixes the eight Nether stem/hyphae identities' deliberate absence from the
fire odds table and from lava ignition: their ordinary-fire encouragement/flammability is `0/0`.
Their `logs` membership does not make them burnable.
`BLK-CORAL-BLOCK-001` fixes the same deliberate `0/0` ordinary-fire odds and lava-ignition
absence for every live and dead coral-block identity; none is a vanilla furnace fuel.
`BLK-CORAL-PLANT-001` fixes that same `0/0` and no-lava-ignition boundary for all thirty upright,
floor-fan and wall-fan identities. None of the twenty item forms is a vanilla furnace fuel.
`BLK-FLOWER-POT-001` fixes the same `0/0` fire odds and no-lava-ignition result for all 37 pot
blocks; the sole flower-pot item has burn time zero.
`BLK-COPPER-FULL-001` fixes `0/0` fire odds, no lava ignition and burn time zero for all 24
block-item pairs. Neither weather age nor wax changes these negative fire/fuel boundaries.
`BLK-SAPLING-001` fixes `0/0` block fire odds and no lava ignition for all eight identities, while
direct item `saplings` membership makes every block item a 100-tick vanilla furnace fuel.
`BLK-BAMBOO-001` fixes stalk fire odds `60/60`, sapling odds `0/0`, lava ignition for both and a
direct 50-tick bamboo fuel entry. Bamboo is absent from the composter table.
`BLK-ANCIENT-DEBRIS-001` fixes `0/0` fire odds, no lava ignition and burn time zero. Its placed
block has no fire immunity branch; the separate default item component rejects exactly the eight
...
`BLK-TORCHFLOWER-CROP-001` fixes crop fire odds `0/0` and mature-flower odds `60/100`; neither
item is a furnace fuel. Torchflower seeds compost at chance 0.3 and the flower at 0.85 through the
