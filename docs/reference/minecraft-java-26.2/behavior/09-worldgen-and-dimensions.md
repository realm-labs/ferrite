...
`BLK-HONEY-001`,
`BLK-NETHER-WART-BLOCK-001`, `BLK-WARPED-WART-BLOCK-001`, `BLK-NETHER-SPROUTS-001`, `BLK-NETHER-ROOTS-001`, `BLK-NETHER-WART-001`, `BLK-NETHER-STEM-001`, `BLK-CORAL-BLOCK-001`, `BLK-CORAL-PLANT-001`, `BLK-FLOWER-POT-001`, `BLK-COPPER-FULL-001`, `BLK-SAPLING-001`, `BLK-BAMBOO-001`, `BLK-ANCIENT-DEBRIS-001`, `BLK-STEM-CROP-001`, `BLK-SOUL-SAND-001`,
`BLK-MAGMA-001`, `BLK-TORCHFLOWER-CROP-001`;
...
`EXP-BLK-057`, `EXP-BLK-059`, `EXP-BLK-060`, `EXP-BLK-061`, `EXP-BLK-062`, `EXP-BLK-064`, `EXP-BLK-065`, `EXP-BLK-066`, `EXP-BLK-067`, `EXP-BLK-068`, `EXP-BLK-069`, `EXP-BLK-070`, `EXP-BLK-071`, `EXP-BLK-072`, `EXP-BLK-073`, `EXP-BLK-074`, `EXP-BLK-075`, `EXP-BLK-076`, `EXP-BLK-077`, `EXP-BLK-079`
...
`BLK-BEACON-STORAGE-001` owns 131 gold cells in 16 bastion templates, 28 gold cells in 12
ruined-portal templates and two diamond cells in mansion secret rooms, plus the weighted bastion
gold final-state connector. It also owns five bastion gold-replacement joins, strict 0.3
ruined-portal removal, the eight-cell monument core and gated gold/diamond dripstone diagnostics;
named pipeline, structure and jigsaw owners retain selection, RNG, transforms and write order.
`BLK-RAW-STORAGE-001` owns raw copper/iron as the strict `<0.02` third-draw outputs of admitted
Overworld ore-vein cells at inclusive Y 0..50/-60..-8. Both identities can then be replaced by the
three carvers using `overworld_carver_replaceables`; raw gold joins neither path, and none of the
three occurs in 1,212 locked templates. `WGEN-PIPELINE-001` retains density, RNG, phase, mask,
aquifer and write ordering.
`BLK-LAPIS-BLOCK-001` owns the single live state in mansion template `2x2_a3`, at local
...
`BLK-AMETHYST-BLOCK-001` owns state 23402 as the sole amethyst-geode inner-layer provider, budding
...
`BLK-CALCITE-SMOOTH-BASALT-001` owns calcite/smooth-basalt geode middle/outer providers, calcite's
multiface, spring, carver and surface-noise identities, and both blocks' sculk replacement role.
...
`BLK-BRICKS-001` owns 2,558 raw state-2340 cells in 31 templates: 2,526 across 27 trail-ruins
...
final writes.
`BLK-MUD-BRICKS-001` owns 3,870 raw state-7759 cells across 40 trail templates. The 3,867
houses-processed cells and 19 connector final states join the below-`0.1` aging test; three
tower-top cells skip it. The trail owner retains replacement, RNG, clipping and final writes.
`BLK-PURPUR-BLOCK-001` owns 2,233 raw cells across all 20 End-city templates, separating 2,212
cells in 19 reachable inputs from 21 in dead `tower_floor`; the End-city owner retains all writes.
...
`BLK-WARPED-WART-BLOCK-001` owns state 20959 as the paired warped-forest surface result and both
warped-fungus hats. The exact Nether-wart-only test keeps their retained vine threshold at zero;
twisting vines accept state 20959 only as support. Wart-tag closure adds Nether-carver replacement,
and all 1,212 templates contain zero cells. The pipeline owner retains traversal, RNG and writes.
`BLK-NETHER-SPROUTS-001` owns state 20961 as the fixed provider of natural width/height 8/4 and
bonemeal 3/1 Nether-forest-vegetation records. The placed record uses layer count 4 in warped
forests; warped-nylium bonemeal invokes the sprouts record after warped vegetation and before the
bound-8 twisting-vines draw. Reloadable tags also admit tree/mushroom overwrite; all 1,212
templates contain zero cells. The pipeline owner retains attempt draws, gates and writes.
`BLK-NETHER-ROOTS-001` owns root states 20960/21031 in the crimson 87/11/1-of-99 and warped
85/1/13/1-of-100 natural and bonemeal vegetation providers, including layer counts 6/5 and nylium
call order. It also owns the crimson-only fixed-state simple feature behind the soul-sand-valley
count-96 full-height/triangular-offset patch. Tree/mushroom tags admit overwrite, all four root/pot
states have zero template cells, and generic provider/modifier traversal remains with the pipeline.
`BLK-NETHER-WART-001` owns the 20 default age-zero crops in each complete fortress
`CastleStalkRoom` and the 12 age-three cells split 6/1/5 across the three equal-weight rigid
bastion center templates. The housing processor has no wart-matching rule. Fortress piece
selection/orientation/clipping and jigsaw assembly/writes remain with their structure owners.
`BLK-NETHER-STEM-001` owns the unstripped default-Y stem state in each crimson/warped ordinary
and planted huge-fungus record. Heights are uniform 4..13 with a 1/12 double branch; only natural
generation takes the 0.06 huge-form draw, whose four layer corners each use a `<0.1` draw. No
hyphae, stripped or horizontal state is generated, and all eight identities have zero cells in
the 1,212 structure templates; base/bounds/replacement/write order remains with the pipeline.
`BLK-CORAL-BLOCK-001` owns the live `coral_blocks` tag's exact five-member order and exclusion of
all dead states. Each tree, claw or mushroom invocation samples one live member uniformly and
uses that default state throughout the shared exact-water/coral replacement traversal and
decoration helper. `warm_ocean_vegetation` orders those three features and its placed wrapper uses
factor/ratio 400/20, in-square, ocean-floor heightmap and biome admission. Dead blocks are never
generated, and all ten identities have zero cells in 1,212 structure templates; feature
selection, traversal, decoration RNG and writes remain with `WGEN-PIPELINE-001`.
`BLK-CORAL-PLANT-001` owns the five live upright members in `coral_plants`, the following five live
floor fans in flattened `corals`, and the five live wall fans in `wall_corals`. At each admitted
coral-feature cell, the shared decorator uses a `<0.25` draw plus uniform `corals` selection for
the top state, then visits north/east/south/west and, at `<0.2` with exact water, selects a uniform
wall member facing that direction. All selections begin waterlogged; dead forms are excluded.
Warm-ocean bonemeal is an item-use path rather than chunk generation, and all thirty identities
have zero cells in 1,212 structure templates. Feature traversal, admission and writes remain with
`WGEN-PIPELINE-001`.
`BLK-FLOWER-POT-001` owns 120 raw cells in 55 of the 1,212 bundled templates: village 80,
woodland mansion 36, trial chambers 3 and igloo 1. Thirteen identities occur, led by potted cactus
...
`WGEN-PIPELINE-001`.
`BLK-SAPLING-001` fixes 45 placed-feature references which use stage-zero saplings only as
`would_survive` predicates, plus all-eight replaceability in four huge-fungus configurations. The
template scan finds two stage-one acacia cells in one savanna-library template and 58 stage-zero
dark-oak cells in one mansion template; all other species have zero cells. Live stage-one growth
selects configured tree features through the exact small/flower/mega transaction before their
independently owned geometry runs.
`BLK-BAMBOO-001` fixes the two `BambooFeature` configurations: jungle's rarity-four zero-podzol
path and bamboo-jungle's noise-count 0.2-podzol path. The feature draws height 5..16, optionally
rewrites a radius-1..4 substrate disk, writes age-one stalk/top states and preserves the locked
unsupported-success/first-obstacle overwrite quirks. Neither block occurs in any template.
`BLK-ANCIENT-DEBRIS-001` fixes size-3 large and size-2 small scattered ores against the exact
three-member base-stone-Nether closure. Large uses trapezoid absolute Y 8..24; small uses uniform
above-bottom/below-top 8, and all five Nether biomes list them in that order. Each of `0..size`
attempts consumes six offset floats, performs chance-one six-face air rejection without another
float, ignores its flags-2 write and leaves the feature true. No template or processor names it.
`BLK-STEM-CROP-001` fixes all four forms as replaceable in the planted and nonplanted crimson and
warped huge-fungus configurations. Six ordinary/zombie village farm processors replace wheat with
