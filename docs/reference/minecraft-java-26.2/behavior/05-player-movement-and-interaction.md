...
- **Behavior and timing:** `PLY-MOVE-001` specifies ordinary dynamics from selected float input through jump, acceleration, collision, gravity and drag. `PLY-MOVE-VALIDATE-001` separately specifies coordinate/status message selection, server collision probing and teleport convergence; accepted packets snap to exact target and adopt packet on-ground/horizontal flags after validation. `PLY-INPUT-001` owns the still-open key/button-to-float and posture transform before travel.
- **Boundaries and quirks:** Packet loss, latency, and client FPS do not change server gameplay tick progression. Coordinate packet cadence and input-intent cadence are independent. More than five coordinate packets changes the anti-cheat multiplier to one; finite vertical moved-wrongly residual is discarded by an OR-condition quirk; pending movement may rotate but not translate the player.
- **Verification owner (`PLY-INPUT-001`, `PLY-MOVE-001`, `PLY-MOVE-VALIDATE-001`; `EXP-PLY-001`, `EXP-PLY-005`, `EXP-PLY-006`):** Ordinary dynamics and coordinate validation/convergence are source-specified. Input shaping remains the explicit unfinished semantic before this parent is fully closed.
